...
if "session_config" not in st.session_state:
    st.session_state.session_config = {"system_prompt": "", "search_top_k": 5, "max_turns": 6, "retrieval_on": True}
...
def load_session(session_id):
    try:
        ...
        if resp.status_code == 200:
            ...
            st.session_state.session_config = s.get("config", {"system_prompt": "", "search_top_k": 5, "max_turns": 6, "retrieval_on": True})
    except Exception:
        pass
...
with st.sidebar:
    ...
    if st.session_state.current_session_id:
        with st.expander("会话配置", expanded=False):
        ...
        if prompt := st.chat_input("输入你的问题..."):
            ...
            if stop_clicked:
                ...
                with st.chat_message("assistant"):
                    ...
                    st.session_state.messages.append({"role": "assistant", "content": answer, "sources": sources, "category": category})
...
# ── Tab 2: 文档管理 ────────────────────────────────────
    ...
    with col2:
        st.subheader("索引")
        if st.button("🔄 重建索引", key=f"reindex_{cat_key}", use_container_width=True):
            with st.spinner("重建中..."):
                resp = requests.post(f"{API}/documents/reindex", json={"category": cat_key}, timeout=60)
                if resp.status_code == 200:
                    data = resp.json()
                    st.success(f"已完成: {data['stats']}")
                else:
                    st.error("失败")
        ...
        if uploaded_file and st.button("上传并建索引", key="upload_btn", use_container_width=True):
            with st.spinner("上传中..."):
                resp = requests.post(
                    f"{API}/upload",
                    files={"file": (uploaded_file.name, uploaded_file.getvalue())},
                    data={"category": cat_key, "chunk_size": str(chunk_size), "chunk_overlap": str(chunk_overlap)},
                )
                if resp.status_code == 200:
                    data = resp.json()
                    st.success(f"已导入: {data['chunks']} chunks")
                else:
                    st.error("上传失败")
    ...
    if st.session_state.edit_file:
        ...
        st.subheader(f"✏️ 编辑: {ef['filename']}")
        ...
        with c1:
            if st.button("💾 保存", key="save_doc"):
                with st.spinner("保存中..."):
                    ...
                    if resp.status_code == 200:
                        data = resp.json()
                        st.success(f"已保存 → {data['chunks']} chunks")
                        st.session_state.edit_file = None
                        st.session_state.edit_content = ""
                        st.rerun()
                    else:
